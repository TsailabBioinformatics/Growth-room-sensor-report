...
'''
    load required data
'''
configData = readJson("/data/rpi0/config.json")
#RPI_Details = readJson("/data/SensorsData/RPI-Details.json")

'''  firebase db-key  '''
cred = credentials.Certificate("/data/rpi0/db-key.json")
...
@app.route('/rpi0',methods=['GET','POST'])
def default():
    return "connected"
...
def emailsavedata():
    ...
    #    path = "/data/SensorsData/RPI-Details.json"
...
def storedata():
    ...
    imgInMem.save("/data/rpi0/"+name)
    
    storage.child("{}/{}".format(storageBucket,name)).put("/data/rpi0/"+name)
    os.remove("/data/rpi0/"+name)
...
if __name__ == '__main__':
    ...
    app.run(host='172.30.18.104', port=8072, debug=True)